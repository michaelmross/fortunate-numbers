# Code for "The Distribution of Two-Sided Fortunate Numbers and the
...
## superseded/  (provenance only — do not use)
- **fortunate_analyze.py**, **_v2.py** — offset-space (u ~ Exp(e^γ))
  analyzers from before the candidate-count form became primary.
- **fortunate_analyze_v3.py** — has a k-label/T-array misalignment in its
  band table and argmax; fixed in v31. Kept to document the correction.
- **expander.py** — five-line seed of expand_primes.py.
- **fortunate_sequence.py** — early one-sided sympy table (upper side
  only, different quantity); not part of the measurement pipeline.
